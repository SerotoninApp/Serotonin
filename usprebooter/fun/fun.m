...
//    
//    uint64_t pr_proc = kread64(proc_ro + off_p_ro_pr_proc);
//    NSLog(@"[i] self proc->proc_ro->pr_proc: 0x%llx", pr_proc);
//    
//    uint64_t pr_task = kread64(proc_ro + off_p_ro_pr_task);
//    NSLog(@"[i] self proc->proc_ro->pr_task: 0x%llx", pr_task);
//    
//    uint64_t itk_space_pac = kread64(pr_task + off_task_itk_space);
//    uint64_t itk_space = itk_space_pac | base_pac_mask;
//    NSLog(@"[i] self task->itk_space: 0x%llx", itk_space);
//    
//    uint32_t port_index = MACH_PORT_INDEX(port_name);
//    uint32_t table_size = kread32(itk_space + 0x14);
//    NSLog(@"[i] table_size: 0x%x, port_index: 0x%x", table_size, port_index);
//    if (port_index >= table_size) {
//        NSLog(@"[-] invalid port name? 0x%x", port_name);
//    }
//    
//    uint64_t is_table = kread64_smr(itk_space + off_ipc_space_is_table);
//    NSLog(@"[i] self task->itk_space->is_table: 0x%llx", is_table);
//    
//    uint64_t entry = is_table + port_index * 0x18/*SIZE(ipc_entry)*/;
//    NSLog(@"[i] entry: 0x%llx", entry);
//    
//    uint64_t object_pac = kread64(entry + off_ipc_entry_ie_object);
//    uint64_t object = object_pac | base_pac_mask;
//    
...
//    uint64_t kobject = kobject_pac | base_pac_mask;
//    NSLog(@"[i] ipc_port: ip_bits 0x%x, ip_refs 0x%x", ip_bits, ip_refs);
//    NSLog(@"[i] ip_kobject: 0x%llx", kobject);
//    
//    return kobject;
//}
...
uint64_t print_key_value_in_os_dict(uint64_t os_dict) {
    ...
    if(((cur_key = malloc(max_key_len)) != NULL)) {
        ...
        if(os_dict_entry_ptr != 0) {
            os_dict_entry_ptr = os_dict_entry_ptr | base_pac_mask;
            NSLog(@"[i] os_dict_entry_ptr: 0x%llx", os_dict_entry_ptr);
            os_dict_cnt = kread32(os_dict + 0x14/*OS_DICTIONARY_COUNT_OFF*/);
            if(os_dict_cnt != 0) {
                NSLog(@"[i] os_dict_cnt: 0x%x", os_dict_cnt);
                while(os_dict_cnt-- != 0) {
                    kreadbuf(os_dict_entry_ptr + os_dict_cnt * sizeof(os_dict_entry), &os_dict_entry, sizeof(os_dict_entry));
                    //NSLog(@"key: 0x%llx, val: 0x%llx", os_dict_entry.key, os_dict_entry.val);
                    ...
                    string_ptr = string_ptr | base_pac_mask;
                    kreadbuf(string_ptr, cur_key, cur_key_len);
                    NSLog(@"[+] key_str: %s, key_str_len: 0x%x", cur_key, cur_key_len);
...
//                    HexDump(os_dict_entry.val, 100);
                    ...
                    NSLog(@"[+] val_str: %s, val_str_len: 0x%x", cur_val, cur_val_len);
                    free(cur_val);
                }
            }
        }
...
//        NSLog(@"[i] nvram_entry: 0x%x", nvram_entry);
//
//        uint64_t of_dict = get_of_dict(nvram_entry);
//        NSLog(@"[i] of_dict: 0x%llx", of_dict);
//        
...
//    NSLog(@"[i] Kernel base: 0x%llx", kbase);
//    NSLog(@"[i] Kernel slide: 0x%llx", kslide);
//    uint64_t kheader64 = kread64(kbase);
//    NSLog(@"[i] Kernel base kread64 ret: 0x%llx", kheader64);
//    
//    pid_t myPid = getpid();
//    uint64_t selfProc = getProc(myPid);
//    NSLog(@"[i] self proc: 0x%llx", selfProc);
//    
//    funUcred(selfProc);
//    funProc(selfProc);
...
//    NSLog(@"[i] var_mobile_vnode: 0x%llx", var_mobile_vnode);
    
//    uint64_t var_tmp_vnode = findChildVnodeByVnode(var_vnode, "tmp");
//    NSLog(@"[i] var_tmp_vnode: 0x%llx", var_tmp_vnode);
    
//    uint64_t var_mobile_library_vnode = findChildVnodeByVnode(var_mobile_vnode, "Library");
//    NSLog(@"[i] var_mobile_library_vnode: 0x%llx", var_mobile_library_vnode);
//    uint64_t var_mobile_library_preferences_vnode = findChildVnodeByVnode(var_mobile_library_vnode, "Preferences");
//    NSLog(@"[i] var_mobile_library_preferences_vnode: 0x%llx", var_mobile_library_preferences_vnode);
//    sleep(1);
...
//    NSLog(@"mntPath directory list: %@", dirs);
    ...
    NSLog(@"mount ret: %d", mount("apfs", mntpath, 0, &mntargs))
    ...
    for(NSString *dir in dirs) {
        ...
        NSLog(@"open %s, ret: %d", mydir.UTF8String, fd_open);
        if(fd_open != -1) {
    ...
    NSLog(@"open ret: %d", open([mntPath stringByAppendingString:@"/mobile/Library"].UTF8String, O_RDONLY));
    NSLog(@"open ret: %d", open([mntPath stringByAppendingString:@"/containers"].UTF8String, O_RDONLY));
    NSLog(@"open ret: %d", open([mntPath stringByAppendingString:@"/mobile/Library/Preferences"].UTF8String, O_RDONLY));
    NSLog(@"open ret: %d", open("/var/containers/Shared/SystemGroup/systemgroup.com.apple.mobilegestaltcache/Library/Caches", O_RDONLY));
