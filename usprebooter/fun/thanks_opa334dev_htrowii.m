...
char* funVnodeRead(char *file) {
    NSLog(@"attempting opa's method");
    NSLog(@"reading %s", file);
    int file_index = open(file, O_RDONLY);
    if (file_index == -1)  {
        NSLog(@"to file nonexistent");
        return -1;
    }
...
u64 funVnodeOverwrite2(char* to, char* from) {
    ...
    NSLog(@"it is writable!");
    memcpy(to_file_data, from_file_data, from_file_size);
    NSLog(@"[i] msync ret: %dn", msync(to_file_data, to_file_size, MS_SYNC));
//    funVnodeSave(to);
...
u64 funVnodeOverwriteWithBytes(const char* filename, off_t file_offset, const void* overwrite_data, size_t overwrite_length, bool unmapAtEnd) {
    ...
    if (file_size < file_offset + overwrite_length) {
        close(file_index);
        NSLog(@"[-] Offset + length is beyond the file size!");
        return -1;
    }
    
//     mmap as read-write
    NSLog(@"mmap as read only");
    char* file_data = mmap(NULL, file_size, PROT_READ, MAP_PRIVATE, file_index, 0);
    if (file_data == MAP_FAILED) {
        NSLog(@"failed mmap... try again");
        close(file_index);
    ...
    NSLog(@"Writing data at offset %lld", file_offset);
...
void overwriteWithFileImpl(NSURL *replacementURL, const char *pathToTargetFile) {
    ...
    NSLog(@"Shortened URL: %s", shortenedURL);
